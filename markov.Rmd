...
```
# Problem 2 
In the setting of Problem 1, assume that the random digit generation stops at the first occurrence
of the TLN (i.e., that the state S4 of the Markov chain is now absorbing). In this problem, you will estimate the average
length of such sequences (i.e., the average time till absorption in the Markov chain).

## 1
Make necessary amendments to the transition probabilities matrix P above and solve the corresponding system
to find the expected time E(T ) till absorption
...
## 2
Estimate numerically the expected length E(T) till the first occurrence of the TLN by running a sufficiently large
...
```{r}
res <- get_expected_length(1000000)
mean(res)
...
Confidence: $$
1 - \alpha = 0.95 \leq P(|\hat T_n - T_n|<c) \\
 P(\frac{|\hat p_n - p_n|\sqrt{N}} {\sigma} \leq \frac {c\sqrt{N}} {\sigma} ) \approx 1- 2Ф(-\frac {\sqrt{N}} \sigma 0.03)  \\
Ф(-\frac {\sqrt{N}} \sigma 0.03) = \frac \alpha 2 = 0.025 \\
\sigma_{theoretical} = 0.4823 \\
...
th_sd_abs = 1000
# th_sd_abs  = mean(get_expected_length(1e7))
th_mu_abs = 1000
th_min_N_abs = 2e4
```
